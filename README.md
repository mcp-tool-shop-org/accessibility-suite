...
> **Run from source.** Nothing in this suite is published to npm or PyPI yet. Every command
> below installs from this repository. The package names in each tool's `package.json` and
> `pyproject.toml` are the intended published names, not links you can install today.
...
|---------|-------------|-------|--------------|
| [a11y-lint](src/a11y-lint/) | Accessibility linter for CLI output -- validates error messages follow accessible patterns | Python 3.10+ | source only |
| [a11y-ci](src/a11y-ci/) | CI gate for accessibility scorecards with regression detection and allowlists | Python 3.10+ | source only |
| [a11y-assist](src/a11y-assist/) | Low-vision-first CLI assistant with five accessibility profiles | Python 3.10+ | source only |
| [a11y-evidence-engine](src/a11y-evidence-engine/) | Headless HTML scanner with prov-spec provenance records | Node.js 18+ | source only |
| [a11y-mcp-tools](src/a11y-mcp-tools/) | MCP server for accessibility evidence capture and diagnosis | Node.js 18+ | source only |
...
Or, after `npm --prefix src/a11y-mcp-tools link`:

...
大多数辅助功能工具只能指出“您有 12 个问题”。辅助功能套件做得更多：它记录了经过验证的测试结果，在持续集成（CI）流水线中检测回归问题，并提供针对低视力、屏幕阅读器、阅读障碍和认知负荷的修复建议。

该套件涵盖整个生命周期：检查命令行输出以查找符合辅助功能标准的模式，扫描 HTML 以查找符合 WCAG 标准的违规情况并记录其来源，在 CI 中强制执行质量控制，并通过 MCP 将所有内容暴露出来，以便人工智能助手参与到修复过程中。

**核心原则：**

- **以证据为基础，而非断言**：每个发现都伴随一个包含 SHA-256 完整性校验的“prov-spec”来源记录。
- **以低视力为优先的输出**：所有命令行工具都使用 `[OK]/[WARN]/[FAIL] + 问题/原因/解决方案` 的格式。
- **确定性**：相同的输入始终产生相同的输出；没有网络调用，没有随机性。
- **原生 CI 支持**：提供退出码、 scorecard JSON 格式以及用于自动化流水线的代码提交评论。
...
| 项目 | 描述 | Stack | 包 |
| --------- | ------------- | ------- | --------- |
| [a11y-lint](src/a11y-lint/) | 用于命令行输出的辅助功能检查器，用于验证错误消息是否符合辅助功能标准。 | Python 3.10+ | [PyPI](https://pypi.org/project/a11y-lint/) |
...
| [a11y-assist](src/a11y-assist/) | 具有五种辅助功能配置的命令行辅助工具，优先考虑低视力用户。 | Python 3.10+ | [PyPI](https://pypi.org/project/a11y-assist/) |
| [a11y-evidence-engine](src/a11y-evidence-engine/) | 无头 HTML 扫描器，具有“prov-spec”来源记录。 | Node.js 18+ | [npm](https://www.npmjs.com/package/@mcptoolshop/a11y-evidence-engine) |
...
2. **a11y-evidence-engine** 扫描 HTML 文件，并输出包含“prov-spec”来源记录的发现结果。
3. **a11y-ci** 消费 scorecard，强制执行阈值，检测回归问题，并生成代码提交评论。
4. **a11y-mcp-tools** 将证据捕获和诊断包装为 MCP 工具，以便与人工智能助手集成。
5. **a11y-assist** 接收发现结果（结构化的 JSON 或原始文本），并生成针对五种辅助功能配置的修复建议。
6. **a11y-demo-site** 将所有内容整合到一个可运行的示例中，其中包含故意添加的违规情况。

**共享接口：**

- `cli.error.schema.v0.1.json`：所有 Python 工具使用的结构化错误格式。
- `evidence.bundle.schema.v0.1.json`：包含来源链的证据包。
- `.a11y_artifacts/`：用于 CI 流水线的统一工件目录。
- prov-spec 方法 ID：用于每个来源步骤的稳定、版本化的标识符。
...
| Tool | 描述 |
| ------ | ------------- |
| `a11y.evidence` | 从HTML、命令行日志或其他输入源捕获具有防篡改功能的证据包。 |
| `a11y.diagnose` | 对带有来源验证的证据包进行WCAG规则检查。 |
...
请参阅[GETTING_STARTED.md](GETTING_STARTED.md)，其中包含Azure DevOps的示例和故障排除指南。
...
| 文档 | 描述 |
| ---------- | ------------- |
| [HANDBOOK.md](HANDBOOK.md) | 深入了解架构、集成模式和开发指南。 |
| [GETTING_STARTED.md](GETTING_STARTED.md) | 三步本地配置、CI模板和故障排除。 |
| [CHANGELOG.md](CHANGELOG.md) | 以Keep a Changelog格式记录的发布历史。 |
